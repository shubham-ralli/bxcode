...
class MenuController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Get Custom Post Types for menu items
        $cpts = [];
        if (Schema::hasTable('custom_post_types')) {
            $cpts = DB::table('custom_post_types')->where('active', 1)->get();
        }
        foreach ($cpts as $cpt) {
            $accordions[] = [
        ...
        // 4. Taxonomies (Categories, Tags, and Custom Taxonomies if ACF is active)
        // Get distinct taxonomies from Tags table
        $taxonomiesQuery = \App\Models\Tag::select('taxonomy')->distinct();

        // If ACF plugin is not active, only show standard taxonomies
        if (!Schema::hasTable('custom_taxonomies')) {
            $taxonomiesQuery->whereIn('taxonomy', ['category', 'post_tag']);
        }

        $taxonomies = $taxonomiesQuery->pluck('taxonomy');
