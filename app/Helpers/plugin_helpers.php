...
if (!function_exists('plugin_is_active')) {
    ...
    function plugin_is_active($pluginSlug = null)
    {
        ...
        return \App\Models\Setting::get("plugin_{$pluginSlug}_active", '1') === '1';
    }
}
...
if (!function_exists('supports_meta_box')) {
    ...
    function supports_meta_box($type, $feature)
    {
        ...
        if (!isset($cptCache[$type]) && \Illuminate\Support\Facades\Schema::hasTable('custom_post_types')) {
        ...
        if (!isset($cptCache[$type])) {
            return false; // Table doesn't exist or type not found
        }
...
/**
 * Add custom code/comments to head and footer
 */
if (!function_exists('inject_custom_head_code')) {
    function inject_custom_head_code()
    {
        $headCode = \App\Models\Setting::get('custom_head_code', '');
        if (!empty($headCode)) {
            echo $headCode . "\n";
        }
    }
...
if (!function_exists('inject_custom_footer_code')) {
    function inject_custom_footer_code()
    {
        $footerCode = \App\Models\Setting::get('custom_footer_code', '');
        if (!empty($footerCode)) {
            echo $footerCode . "\n";
        }
    }
...
// Hook into head and footer
add_action('bx_head', 'inject_custom_head_code', 999);
add_action('bx_footer', 'inject_custom_footer_code', 999);
