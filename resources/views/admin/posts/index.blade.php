            ...
            <tr class="hover:bg-gray-50 transition-colors group">
                ...
                <td class="px-5 py-4">
                    <div class="text-sm font-medium text-gray-900">
                        ...
                        <div class="mt-1 opacity-0 group-hover:opacity-100 transition-opacity text-xs space-x-2">
                            @if($post->status === 'trash')
                                <!-- Trash Actions: Restore | Delete -->
                                <a href="{{ route('admin.posts.restore', $post->id) }}"
                                    class="text-green-600 hover:text-green-900 font-medium">Restore</a>
                            ...
                            @elseif($post->status === 'draft')
                                ...
                                <span class="text-gray-300">|</span>
                                <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <button type="submit" class="text-green-600 hover:text-green-900 font-medium">Publish</button>
                                </form>
                            @else
                                <!-- Public/Private Actions: Edit | View -->
                                <a href="{{ route('admin.posts.edit', ['post' => $post->id, 'action' => 'edit']) }}"
                                    class="text-indigo-600 hover:text-indigo-900 font-medium">Edit</a>
                ...
                <td class="px-5 py-4 text-sm whitespace-nowrap text-right">
                    ...
                    @else
                        <a href="{{ route('admin.posts.restore', $post->id) }}"
                            class="text-green-600 hover:text-green-900 font-medium mr-3">Restore</a>
                    @endif
                </td>
            </tr>
        @empty
